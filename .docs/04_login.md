...
> **⚠ 実装注記:** このドキュメント内の「Supabase Auth」記述はかつての方針。現在は **Convex Auth + GitHub Provider** で実装する。フロー（GitHub OAuth → username取得 → account作成）は変わらない。実装詳細は[security](../.agents/skills/quine-implement/references/security.md)と[Convex](../.agents/skills/quine-implement/references/convex.md)を参照。
