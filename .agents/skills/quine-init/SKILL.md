...
- **mockup / AGENTS.md / .docs/ を変更しない**（確定済み）
...
`.Codex/settings.local.json` に `"enableAllProjectMcpServers": true` を追加。

⚠️ **MCP は次のセッションから利用可能**（現セッションでは使えない）。`Codex mcp list` で `convex - ✓ Connected` を確認できれば OK。
