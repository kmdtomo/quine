// ============================================
// prism.js - WebGL Prism 背景 (vanilla JS port)
// 元: React Bits (Prism-TS) を ogl + vanilla JS に移植
// 使い方:
//   import { createPrism } from './prism.js';
//   const cleanup = createPrism(containerEl, { animationType: 'rotate', ... });
//   // 破棄時: cleanup();
// ============================================
...
export function createPrism(container, options = {}) {
  ...
  const fragment = /* glsl */ `
    ...
    // 角丸キューブ
    float sdStack(vec3 p){
      float s = uBaseHalf * 0.5;
      float r = 0.12; // 角丸半径
      vec3 q = abs(p) - vec3(s, s, s) + vec3(r);
    ...
    void main(){
      ...
      // ブランド緑 (#07DE81) を下部にブレンド。青はほぼ消して金との融合を保つ
      vec3 brand = vec3(0.4, 1.0, 0.15); // 緑優位・青ほぼゼロ
      float vy = gl_FragCoord.y / iResolution.y; // 0=bottom, 1=top
      float greenWeight = smoothstep(0.55, 0.0, vy) * 0.6;
      vec3 tinted = clamp(col * brand * 1.6, 0.0, 1.0);
      tinted.b *= 0.1; // 青を徹底的に抑制
      col = mix(col, tinted, greenWeight);
      col.b = mix(col.b, col.b * 0.7, greenWeight); // 全体の青も下部で減衰
